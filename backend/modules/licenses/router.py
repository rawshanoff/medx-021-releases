...
from backend.modules.licenses.schemas import LicenseUpload, LicenseStatus
...
async def license_status(_user=Depends(require_roles(UserRole.ADMIN, UserRole.OWNER, UserRole.RECEPTIONIST, UserRole.DOCTOR, UserRole.CASHIER))):
    payload = license_manager.load_license()
    if "error" in payload:
        return LicenseStatus(active_features=[], error=payload["error"])
    return LicenseStatus(active_features=license_manager.get_active_features(), error=None)
...
async def upload_license(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save license: {e}")
