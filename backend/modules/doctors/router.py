...
from backend.modules.doctors.schemas import DoctorCreate, DoctorRead, DoctorServiceCreate, DoctorUpdate
from backend.modules.doctors.models import AuditLog
...
async def create_doctor(
    ...
    result = await db.execute(select(Doctor).options(selectinload(Doctor.services)).where(Doctor.id == new_doctor.id))
...
async def list_doctors(
    ...
    result = await db.execute(select(Doctor).options(selectinload(Doctor.services)).order_by(Doctor.full_name))
...
async def update_doctor(
    ...
    if hasattr(doctor_update, 'queue_prefix') and doctor_update.queue_prefix is not None:
    ...
    result = await db.execute(select(Doctor).options(selectinload(Doctor.services)).where(Doctor.id == doctor_id))
    return result.scalars().first()
...
async def delete_doctor(
    ...
    try:
        ...
        await db.execute(select(QueueItem).where(QueueItem.doctor_id == doctor_id)) # Check if any existence? No need, just delete.
        # Actually in SQLAlchemy asyncio we construct a delete statement
...
async def get_history(
    ...
    return [{"time": l.timestamp.strftime("%Y-%m-%d %H:%M:%S") if l.timestamp else "", "action": l.action, "details": l.details} for l in logs]
