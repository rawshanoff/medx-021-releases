...
export default function MixedPaymentModal({ isOpen, onClose, totalAmount, onConfirm }: MixedPaymentModalProps) {
    ...
    return (
        <Modal
            ...
            description={t('reception.total_amount') + `: ${totalAmount.toLocaleString()} ${t('common.currency')}`}
            width={560}
        >
            <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div className="space-y-1">
                    <label className="text-[12px] font-medium text-foreground">{t('reception.transfer')}</label>
                    <Input
                        type="number"
                        min={0}
                        inputMode="numeric"
            ...
            >
                {remaining > 0 ? `${t('reception.remaining')}: ${remaining.toLocaleString()} ${t('common.currency')}` : null}
                {remaining < 0 ? `${t('reception.excess')}: ${Math.abs(remaining).toLocaleString()} ${t('common.currency')}` : null}
