  ...
  const refresh = async () => {
    ...
    try {
      const [shiftRes, xRes, zRes] = await Promise.all([
        ...
        client.get('/finance/reports/X').catch((e) => ({ data: { error: e?.response?.data?.detail || String(e?.message || e) } })),
        client.get('/finance/reports/Z').catch((e) => ({ data: { error: e?.response?.data?.detail || String(e?.message || e) } })),
      ]);
          ...
          <div className="rounded-md border border-border bg-card p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div className="text-[13px] font-medium">{t('reports.basic', { defaultValue: 'Базовые отчёты (бесплатно)' })}</div>
              </div>
              <div className="text-[12px] text-muted-foreground">
                {shift ? t('finance.shift_open', { id: shift.id }) : t('finance.shift_closed', { defaultValue: 'Смена закрыта' })}
              </div>
            </div>

            <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-3">
              <div className="rounded-md border border-border bg-background p-3">
                <div className="text-[13px] font-medium">{t('reports.shift_totals', { defaultValue: 'Касса за смену' })}</div>
                <div className="text-[12px] text-muted-foreground">{t('reports.by_payment', { defaultValue: 'По типам оплаты' })}</div>
                <div className="mt-3 grid gap-2 text-[13px]">
                  ...
                  <div className="flex justify-between gap-3 border-t border-border pt-2">
                    <span className="font-medium">{t('reports.total', { defaultValue: 'Итого' })}</span>
          ...
          <div className="rounded-md border border-border bg-card p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div className="text-[13px] font-medium">{t('reports.pro', { defaultValue: 'Профессиональные отчёты (платно)' })}</div>
              </div>
              <div className="text-[12px] text-muted-foreground">
                {t('reports.pro_hint', { defaultValue: 'Эти отчёты доступны после активации PRO лицензии.' })}
              </div>
            </div>

            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { title: t('reports.cash_flow_day', { defaultValue: 'Cash Flow за день' }), desc: t('reports.cash_flow_day_desc', { defaultValue: 'Поступления/расходы/чистая прибыль по дням' }) },
                { title: t('reports.by_doctor', { defaultValue: 'Отчёт по врачам' }), desc: t('reports.by_doctor_desc', { defaultValue: 'Выручка по врачам и услугам' }) },
                { title: t('reports.by_service', { defaultValue: 'Отчёт по услугам' }), desc: t('reports.by_service_desc', { defaultValue: 'ТОП услуг, динамика, средний чек' }) },
                { title: t('reports.debts', { defaultValue: 'Долги и возвраты' }), desc: t('reports.debts_desc', { defaultValue: 'Сводка по задолженностям/возвратам' }) },
              ].map((x) => (
                <div key={x.title} className="rounded-md border border-border bg-background p-3 opacity-60">
