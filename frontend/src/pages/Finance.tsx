    ...
    return (
        <div className="flex h-full flex-col gap-3">
            <div className="flex flex-wrap items-end justify-between gap-2">
                    ...
                    {!shift ? (
                        <Button size="sm" className="h-8 text-xs" type="button" onClick={handleOpenShift} disabled={loading}>
                            <Unlock size={14} /> {t('finance.open_shift')}
                        </Button>
                    ) : (
                        <div className="flex flex-wrap items-center gap-2">
                            <span className="text-[13px] font-medium text-primary">{t('finance.shift_open', { id: shift.id })}</span>
                            <Button variant="secondary" size="sm" className="h-8 text-xs" type="button" onClick={handleCloseShift} disabled={loading}>
                ...
                <div className="min-h-0 flex-1 space-y-3 overflow-auto">
                    ...
                    <div className="rounded-md border border-border bg-card p-2">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="text-[13px] font-medium">{t('finance.cash_flow', { defaultValue: 'Касса / Cash Flow' })}</div>
                            <Button variant="secondary" size="sm" className="h-8 text-xs" type="button" onClick={checkActiveShift} disabled={loading}>
                        ...
                        <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
                            {[
                                ...
                                <div key={x.label} className="rounded-md border border-border bg-background p-2">
                                    <div className="text-[12px] text-muted-foreground dark:text-gray-300">{x.label}</div>
                        ...
                        <div className="mt-2 text-[12px] text-muted-foreground dark:text-gray-300">
                            {t('finance.shift_hint', { defaultValue: 'Это сводка по текущей открытой смене. Дневной Cash Flow будет в PRO‑отчётах.' })}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2 lg:grid-cols-3">
                        ...
                        >
                            <div className="flex gap-2">
                                ...
                                <div className="min-w-0">
                                    <div className="text-[13px] font-medium">{t('finance.new_transaction', { defaultValue: 'Новый расход' })}</div>
                                    <div className="mt-0.5 text-[12px] text-muted-foreground dark:text-gray-300">{t('finance.expense_desc', { defaultValue: 'Описание расхода' })}</div>
                                </div>
                            </div>
                        </button>
                        ...
                        >
                            <div className="flex gap-2">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <div className="mt-0.5 text-[12px] text-muted-foreground dark:text-gray-300">{t('finance.reports_desc', { defaultValue: 'X/Z отчёты + PRO отчёты' })}</div>
                                </div>
                            </div>
                        </button>
                        ...
                        <div className="rounded-md border border-border bg-card p-2 opacity-60">
                            <div className="flex gap-2">
                                ...
                                <div className="min-w-0">
                                    <div className="text-[13px] font-medium">{t('finance.day_cash', { defaultValue: 'Касса за день' })}</div>
                                    <div className="mt-0.5 text-[12px] text-muted-foreground dark:text-gray-300">{t('finance.day_cash_desc', { defaultValue: 'Доступно в PRO отчётах' })}</div>
                                    <Button className="mt-2 h-8 text-xs" variant="secondary" size="sm" type="button" disabled>
                                        <Lock size={14} /> PRO
                                    </Button>
                                </div>
                            </div>
                ...
                <Modal
                    ...
                    description={t('finance.expense_modal_desc', { defaultValue: 'Расход списывается как отрицательная транзакция. Требуется открытая смена.' })}
                    onClose={() => setExpenseOpen(false)}
                    width={860}
                >
                    <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                        <div>
                            <label className="mb-1 block text-[12px] text-muted-foreground dark:text-gray-300">{t('finance.total_amount', { defaultValue: 'Сумма' })}</label>
                            <Input className="h-8 text-xs dark:border-gray-600" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0" />
                        </div>
                        <div>
                            <label className="mb-1 block text-[12px] text-muted-foreground dark:text-gray-300">{t('finance.payment_method', { defaultValue: 'Тип оплаты' })}</label>
                            <div className="flex gap-2">
                                {(['CASH', 'CARD'] as const).map((method) => (
                                    <Button
                                        ...
                                        className={cn(
                                            ...
                                            paymentMethod === method ? "border-destructive bg-destructive/10 text-destructive" : ""
                                        )}
                        ...
                        <div className="md:col-span-2">
                            <label className="mb-1 block text-[12px] text-muted-foreground dark:text-gray-300">{t('finance.expense_desc', { defaultValue: 'Описание расхода' })}</label>
                            <Input
                                value={description}
                                onChange={(e) => setDescription(e.target.value)}
                                placeholder={t('finance.expense_placeholder', { defaultValue: 'Например: канцтовары, такси…' })}
                    ...
                    <div className="mt-3 flex justify-end gap-2">
                        <Button variant="secondary" size="sm" className="h-8 text-xs" type="button" onClick={() => setExpenseOpen(false)}>
                            {t('common.cancel', { defaultValue: 'Отмена' })}
                        </Button>
                        <Button size="sm" className="h-8 text-xs" type="button" onClick={handleExpense} disabled={txLoading || !shift}>
                            {txLoading ? t('finance.processing', { defaultValue: 'Обработка…' }) : t('finance.charge_btn', { defaultValue: 'Списать' })}
                        </Button>
                    </div>
                </Modal>
            </div>
